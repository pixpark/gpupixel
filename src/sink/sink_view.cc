...
SinkView::SinkView(void* parent_view) {
    ...
    gpuPixelView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    [parentNSView addSubview:gpuPixelView positioned:NSWindowBelow relativeTo:nil];
...
#if defined(GPUPIXEL_IOS)
    ObjcView* gpuPixelView = (__bridge ObjcView*)render_view_;
    [gpuPixelView SetInputFramebuffer:framebuffer withRotation:rotation_mode atIndex:texIdx];
#else
    ObjcView* gpuPixelView = (__bridge ObjcView*)render_view_;
    [gpuPixelView SetInputFramebuffer:framebuffer withRotation:rotation_mode atIndex:texIdx];
#endif
