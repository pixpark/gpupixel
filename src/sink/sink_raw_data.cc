...
std::shared_ptr<SinkRawData> SinkRawData::create() {
  auto sourceRawDataOutput =
      std::shared_ptr<SinkRawData>(new SinkRawData());
...
void SinkRawData::setI420Callbck(RawOutputCallback cb) {
  std::unique_lock<std::mutex> lck(mtx_);
  i420_callback_ = cb;
}

void SinkRawData::setPixelsCallbck(RawOutputCallback cb) {
  std::unique_lock<std::mutex> lck(mtx_);
  pixels_callback_ = cb;
}
...
#if defined(GPUPIXEL_IOS)
void SinkRawData::readPixelsFromCVPixelBuffer() {
  if (kCVReturnSuccess ==
...
void SinkRawData::initTextureCache(int width, int height) {
...
void SinkRawData::initPBO(int width, int height) {
...
// read pixel with pbo
void SinkRawData::readPixelsWithPBO(int width, int height) {
  index = (index + 1) % 2;
  nextIndex = (index + 1) % 2;
