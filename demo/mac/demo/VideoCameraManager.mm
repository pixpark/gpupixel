...
@synthesize horizontallyMirrorFrontFacingCamera =
                _horizontallyMirrorFrontFacingCamera,
               ...
               cameraDevice:(AVCaptureDevice*)cameraDevice;
{
  ...
  cameraProcessingQueue = dispatch_queue_create(
      "com.sunsetlakesoftware.GPUImage.cameraProcessingQueue", NULL);
  audioProcessingQueue = dispatch_queue_create(
      "com.sunsetlakesoftware.GPUImage.audioProcessingQueue", NULL);
  frameRenderingSemaphore = dispatch_semaphore_create(1);

  _frameRate = 0;  // This will not set frame rate unless this value gets set to
                   // 1 or above
  _runBenchmark = NO;
  capturePaused = NO;
  ...
  // Add the video input
  NSError* error = nil;
  videoInput = [[AVCaptureDeviceInput alloc] initWithDevice:_inputCamera
                                                      error:&error];
  ...
  {
    // Despite returning a longer list of supported pixel formats, only RGB,
    // RGBA, BGRA, and the YUV 4:2:2 variants seem to return cleanly
    [videoOutput
        setVideoSettings:
            [NSDictionary
...
// ARC forbids explicit message send of 'release'; since iOS 6 even for
// dispatch_release() calls: stripping it out in that case is required.
// #if ( (__IPHONE_OS_VERSION_MIN_REQUIRED < __IPHONE_6_0) ||
// (!defined(__IPHONE_6_0)) )
...
- (void)rotateCamera {
  ...
  newVideoInput = [[AVCaptureDeviceInput alloc] initWithDevice:backFacingCamera
                                                         error:&error];
...
- (void)setFrameRate:(NSInteger)frameRate;
{
  ...
  if (_frameRate > 0) {
    for (AVCaptureConnection* connection in videoOutput.connections) {
      if ([connection
              respondsToSelector:@selector(setVideoMinFrameDuration:)]) {
    ...
    for (AVCaptureConnection* connection in videoOutput.connections) {
      if ([connection
              respondsToSelector:@selector(setVideoMinFrameDuration:)]) {
        connection.videoMinFrameDuration =
            kCMTimeInvalid;  // This sets videoMinFrameDuration back to default
      }
    }
  }
}
...
- (void)processVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer;
{
  ...
  if (_runBenchmark) {
    ...
    if (numberOfFramesCaptured > INITIALFRAMESTOIGNOREFORBENCHMARK) {
      CFAbsoluteTime currentFrameTime =
          (CFAbsoluteTimeGetCurrent() - startTime);
      totalFrameTimeDuringCapture += currentFrameTime;
      NSLog(@"Average frame time : %f ms",
            [self averageFrameDurationDuringCapture]);
...
{
  return (totalFrameTimeDuringCapture /
          (CGFloat)(numberOfFramesCaptured -
                    INITIALFRAMESTOIGNOREFORBENCHMARK)) *
         1000.0;
}
...
- (void)captureOutput:(AVCaptureOutput*)captureOutput
  ...
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(willOutputSampleBuffer:)]) {
